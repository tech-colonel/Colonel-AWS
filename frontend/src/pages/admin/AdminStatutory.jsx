...
import { STATUTORY_CATEGORIES, CATEGORY_BY_KEY } from '../../lib/statutoryMeta';

/* Admin cross-brand statutory completion — which brand has filed what. */
export default function AdminStatutory() {
  ...
  // Group rows by brand → { brandName, total, filed, byCat: {key:{total,filed,...}} }
  const brands = useMemo(() => {
    const map = new Map();
    (rows || []).forEach(r => {
      if (!map.has(r.brand_id)) map.set(r.brand_id, { id: r.brand_id, name: r.brand_name || 'Brand', total: 0, filed: 0, byCat: {} });
      const b = map.get(r.brand_id);
      b.total += r.total; b.filed += r.filed;
      b.byCat[r.compliance_type] = r;
    });
      ...
      <div style={{ padding: 24, maxWidth: 1200 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Cross-brand filing status — what's done vs pending</div>
          </div>
        ...
        {rows === null ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {brands.map(b => {
              const pct = b.total ? Math.round((b.filed / b.total) * 100) : 0;
              return (
                <div key={b.id} className="glass-card" style={{ padding: 18 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                    <Building2 style={{ width: 17, height: 17, color: '#0748EE' }} />
                    <span style={{ fontFamily: 'Manrope', fontWeight: 800, fontSize: 16, color: 'var(--text-heading)' }}>{b.name}</span>
                    <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10, minWidth: 200 }}>
                      ...
                      <span style={{ fontFamily: 'Barlow', fontWeight: 900, fontSize: 15, color: 'var(--text-heading)', minWidth: 78, textAlign: 'right' }}>{b.filed}/{b.total} · {pct}%</span>
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
                    {STATUTORY_CATEGORIES.map(cat => {
                      const c = b.byCat[cat.key];
                      const total = c?.total || 0;
                      const filed = c?.filed || 0;
                      const p = total ? Math.round((filed / total) * 100) : 0;
                      const done = total > 0 && filed === total;
                      return (
                        <div key={cat.key} style={{ padding: '10px 12px', borderRadius: 12, border: '1px solid var(--card-border)', background: 'var(--page-bg)', opacity: total ? 1 : 0.5 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8 }}>
                            ...
                            <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 700, color: done ? '#059669' : 'var(--text-muted)' }}>
                              {total ? `${filed}/${total}` : '—'}
                            </span>
                          </div>
                          ...
                          {total > 0 && (c.pending > 0 || c.not_due > 0) && (
                            <div style={{ fontSize: 10.5, color: 'var(--text-muted)', marginTop: 6 }}>
                              {c.pending ? `${c.pending} pending · ` : ''}{c.not_due ? `${c.not_due} not due` : ''}{c.not_applicable ? ` · ${c.not_applicable} N/A` : ''}
                            </div>
                          )}
                        </div>
                      );
