...
const ReceivableSheetBrowser = ({ brandId, month, year }) => {
  ...
  }, [brandId, month, year, sheet, status, page, search]);
  ...
  useEffect(() => { setPage(1); }, [sheet, status, search, month, year]);
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr style={{ background: 'var(--page-bg)', borderBottom: '1.5px solid var(--card-border)' }}>
                        {['Date', 'Invoice #', 'Order #', 'AWB', 'Channel', 'Payment', 'Total', 'Collection', 'Return'].map((h) => (
