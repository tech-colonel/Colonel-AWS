...
async function amazonB2CProcessor(
  ...
  try {
    ...
    const x2betaShippingColumns = [
      ...
      { header: 'Vch. No.*', get: r => r['Final Invoice No.'] ? `SHIP-${Number(r['Final Taxable Shipping Value'] || 0) < 0 ? 'CN-' : ''}${r['Final Invoice No.']}` : '' },
      { header: 'Ref. No.', get: r => r['Final Invoice No.'] ? `SHIP-${r['Final Invoice No.']}` : '' },
      { header: 'Ref. Date', get: () => x2betaVchDate },
